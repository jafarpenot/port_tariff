"""The pipeline graph (§6.1) — nodes 1-6, 8, 9 for Stage 2 (node 7,
Verify, is Stage 4 and not wired in yet).

Map and Extract's own internal parallelism (ThreadPoolExecutor, bounded
by a concurrency limit) happens inside their node functions, already
built and tested in map_node.py/extract.py — this graph orchestrates the
macro sequence and the repair loop, and owns the one thing that must be
guaranteed structurally: every extraction passes through Validate before
it can reach the report (§6.2's invariant), with no way to route around
it.
...
from .schemas import CanonicalCharge, ChargeExtraction, PipelineStatus, ReviewReport, ValidationResult, ValidationSeverity, WindowMapResult
from .validate import validate_all

REPAIR_BUDGET = 3  # §6.7, configurable
...
def node_extract(state: PipelineState, config) -> dict:
    ...
    # Repair pass (§6.5): only charges Validate marked invalid, and only
    # while their per-charge repair budget isn't exhausted.
    budget = _cfg(config, "repair_budget", REPAIR_BUDGET)
    repair_counts = dict(state.get("repair_counts", {}))
    validations = state.get("validations", {})
    to_repair: dict[CanonicalCharge, Any] = {}
    issues_by_charge = {}
...
def node_finalize_statuses(state: PipelineState, config) -> dict:
    ...
    Extraction failed here, distinct from Extract's own Unmapped."""
    ...
    return {"pipeline_statuses": statuses}
...
def node_human_approval(state: PipelineState, config) -> dict:
    ...
    suite (not wired up in Stage 2: no schedule registry exists yet to
    write into — see specs/EXTRACTION_SPEC.md §5.2, build stage 5).
    Reject -> stop, keep the report. Either way this graph's only
    responsibility is to pause and record the decision; the caller acts
    on `approved` in the returned state."""
...
def build_graph():
    ...
    graph.add_conditional_edges("validate", route_after_validate, {"extract": "extract", "finalize_statuses": "finalize_statuses"})
